  ...
  <section class="pt-3 mt-3 border-t border-slate-200" aria-labelledby="fill-history-title">
    <div class="flex items-center justify-between gap-2 mb-2">
      ...
      <div class="flex items-center gap-1 flex-shrink-0">
        ...
        <button type="button" @click="emit('replay-export')" class="p-1.5 rounded-md text-slate-500 hover:text-violet-600 hover:bg-violet-50 disabled:opacity-30 disabled:cursor-not-allowed focus-visible:ring-2 focus-visible:ring-violet-500" title="导出运行回放问题包" aria-label="导出运行回放问题包"><Package class="w-3.5 h-3.5" aria-hidden="true" /></button>
    ...
    <div class="mb-3 rounded-xl border border-slate-200 bg-slate-50 p-2.5">
      ...
      <div v-if="quality.topFailures.length" class="mt-2 flex flex-wrap gap-1 text-[10px]">
        <span class="text-slate-500">常见失败：</span>
        <span v-for="([code, count]) in quality.topFailures" :key="code" class="px-1.5 py-0.5 rounded border border-rose-100 bg-rose-50 text-rose-700">{{ code }} × {{ count }}</span>
      </div>
    </div>
    ...
    <details v-if="compatibility.length" class="mb-3 rounded-xl border border-slate-200 bg-white overflow-hidden">
      ...
      <div class="border-t border-slate-100 divide-y divide-slate-100 max-h-48 overflow-y-auto">
      ...
      <details v-for="record in records" :key="record.id" class="group rounded-lg border border-slate-200 bg-white overflow-hidden">
        <summary class="cursor-pointer list-none p-2 hover:bg-slate-50 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-500">
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0">
              <div class="text-xs font-semibold text-slate-700 truncate" :title="record.pageTitle || record.hostname">{{ record.pageTitle || record.hostname || '未知页面' }}</div>
              <div class="text-[10px] text-slate-400 truncate mt-0.5" :title="record.pageUrl">{{ formatTime(record.createdAt) }} · {{ record.hostname || '本地页面' }}</div>
            </div>
        ...
        <div class="border-t border-slate-100 p-2 bg-slate-50/60 space-y-1">
          ...
          <div class="text-[10px] text-slate-400 flex flex-wrap gap-x-2 gap-y-0.5">
            <span v-if="record.verifiedCount !== undefined">验证 {{ record.verifiedCount }}</span>
            <span v-if="record.reviewRequiredCount">重点核对 {{ record.reviewRequiredCount }}</span>
            <span v-if="record.aiMappingCount">AI {{ record.aiMappingCount }}</span>
            <span v-if="record.blockedCount">安全阻断 {{ record.blockedCount }}</span>
          </div>
          ...
          <div v-for="(field, fieldIndex) in record.fields" :key="`${record.id}-${fieldIndex}`" class="text-[10px] rounded bg-white border border-slate-100 px-2 py-1">
            ...
            <p v-if="field.failureCode" class="text-rose-500 mt-0.5">{{ field.failureCode }}</p>
          </div>
        </div>
      </details>
    </div>

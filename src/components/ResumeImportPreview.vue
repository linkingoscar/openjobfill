...
<template>
  <div v-if="parsedResume && healthReport" class="space-y-4 animate-fade-in">
    <div class="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <span v-if="conflicts?.length" class="px-2 py-0.5 bg-amber-100 text-amber-900 rounded-full font-bold text-[11px]">冲突 {{ conflicts.length }} 项</span>
        </div>
      ...
      <div class="p-2.5 rounded-xl border border-blue-200 bg-blue-50 text-[11px] text-blue-900 leading-relaxed">
        导入文件和 AI 结果只作为候选。字段来源、置信度与原文证据会随档案保存；已确认或锁定字段出现不同候选时会进入冲突，不会被自动覆盖。
      </div>
    ...
    <section v-if="conflicts?.length" class="rounded-2xl border border-amber-200 bg-amber-50 overflow-hidden">
      ...
      <div class="divide-y divide-amber-100 max-h-56 overflow-y-auto">
        <div v-for="conflict in conflicts" :key="`${conflict.path}-${conflict.reason}`" class="p-3 grid grid-cols-[150px_1fr_1fr] gap-3 text-[11px]">
          <div><div class="font-bold text-slate-800 break-all">{{ conflict.path }}</div><div class="mt-1 text-amber-700">{{ conflict.reason === 'locked' ? '已锁定，禁止覆盖' : conflict.reason === 'confirmed-different' ? '已确认值不同' : '解析结果不一致' }}</div></div>
    ...
    <section v-if="reviewRows.length" class="rounded-2xl border border-slate-200 bg-white overflow-hidden">
      <header class="px-4 py-2.5 border-b border-slate-200 flex items-center justify-between">
        ...
        <div class="text-[10px] text-slate-500">仅展示前 {{ reviewRows.length }} 项；高置信无冲突项可在保存时采用</div>
      </header>
